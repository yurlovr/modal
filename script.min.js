...
function getSendServer(a){a.preventDefault();var b=new FormData(form.querySelector(".modal-form")),c=new XMLHttpRequest;c.open("POST","file.php",!0);c.send(b);c.onreadystatechange=function(){4===this.readyState&&(200!==c.status?alert("\u0414\u0430\u043d\u043d\u044b\u0435 \u043d\u0435 \u043e\u0442\u043f\u0440\u0430\u0432\u043b\u0435\u043d\u043d\u044b"):(alert("\u0414\u0430\u043d\u043d\u044b\u0435 \u0443\u0441\u043f\u0435\u0448\u043d\u043e \u043e\u0442\u043f\u0440\u0430\u0432\u043b\u0435\u043d\u043d\u044b"),
    form.style.display="none",openForm.style.display="none",overlay.style.display="none",formUserData.style.display="block",getShowMessage(b)))}}
function getSuccessForm(){var a=!1,b=!1,c=!1;userName.onfocus=function(){userName.classList.remove("error-anime")};userName.onchange=function(){!1===/^[\u0430-\u044f\u0451a-z]+$/iu.test(userName.value)?(userName.classList.add("error-anime"),getShowAlert()):(userName.classList.remove("error-anime"),(a=!0,b)&&c&&getShowButton())};userMail.onfocus=function(){userMail.classList.remove("error-anime")};userMail.onchange=function(){!1===/^([A-Za-z0-9_\-\.])+@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/.test(userMail.value)?
    (userMail.classList.add("error-anime"),getShowAlert()):(userMail.classList.remove("error-anime"),c=!0,a&&b&&c&&getShowButton())};userTel.onfocus=function(){userTel.classList.remove("error-anime")};userTel.onchange=function(){!1===/^((\+?7|8)[ \-] ?)?((\(\d{3}\))|(\d{3}))?([ \-])?((\d{3}[\- ]?\d{2}[\- ]?\d{2})|(\d{2}[\- ]?\d{2}[\- ]?\d{3}))$/.test(userTel.value)?(userTel.classList.add("error-anime"),getShowAlert()):(userTel.classList.remove("error-anime"),b=!0,a&&b&&c&&getShowButton())}}
function getShowButton(){form.querySelector(".button-form-modal").style.display="block"}function getShowMessage(a){for(var b=formUserData.querySelectorAll(".header-answer"),c=0,e=$jscomp.makeIterator(a.keys()),d=e.next();!d.done;d=e.next()){d=d.value;var f=document.createElement("p");f.textContent=a.get(d);b[c].insertAdjacentElement("afterend",f);c++}}
function closeFeedBackEsc(a){27===a.keyCode&&(a.preventDefault(),"block"===form.style.display&&(form.style.display="none",overlay.style.display="none",form.classList.remove("error-anime"),clearForm()))}
...
openForm.addEventListener("click",function(){clearForm();setTimeout(function(){form.style.display="block";overlay.style.display="block";userName.focus();getSuccessForm()},5E3)});var closeForm=document.querySelector(".modal-form-feedback-close");closeForm.addEventListener("click",function(){form.style.display="none";overlay.style.display="none";clearForm()});form.addEventListener("submit",getSendServer);window.addEventListener("keydown",closeFeedBackEsc);var buttonOk=document.querySelector(".ok");
